...
\newline \newline  2.\quad For each polynomial $q \in Z[x]$, the matrix
\begin{align}
q(A)=\myvec{q(a) & q(b) & q(c)\\ 0 & q(a) & q(d)\\ 0 & 0 & q(a)} \label{eq:q(A)}
\end{align}
\newline \newline3.\quad If $ A^n=0$ for some positive integer n, then $A^3=0$
\newline \newline4.\quad A commutes with every matrix of the form $\myvec{a^{'} & 0 & c^{'} \\ 0 & a^{'} & 0 \\ 0 & 0 & a^{'}}$ 
\section{Solution}
